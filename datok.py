...
def colors(value):
    patterns = {
        'R1' : '\033[34;1m', 'R2' : '\033[48;5;0011;38;5;004m',
        'G1' : '\033[38;5;20m', 'Y1' : '\033[38;1m',
        'P1' : '\033[33;1m', 'CC' : '\033[0m'
    }
...
def log_replace(value, status='INFO', color='[R2]'):
...
class inject(object):
    ...
    def start(self):
        try:
            ...
            frontend_domains = open(real_path('/sakha.txt')).readlines()
            frontend_domains = filter_array(frontend_domains)
            if len(frontend_domains) == 0:
                self.log('Kesalahan File sakha.txt Silahkan Cek Lagi', color='[R1]')
                return
            self.log('INJECT TELKOMSEL SIAP!!! '.format(self.inject_host, self.inject_port))
            while True:
                socket_client, _ = socket_server.accept()
                socket_client.recv(65535)
                domain_fronting(socket_client, frontend_domains).start()
        except Exception as exception:
            self.log('File sakha.txt Tidak Terbaca'.format(self.inject_host, self.inject_port), color='[R1]')
...
class domain_fronting(threading.Thread):
    ...
    def log(self, value, status='Log', color='[G1]'):
    ...
    def run(self):
        try:
            ...
            self.log('enteni ndisek ijek jimok jaringan'.format(self.proxy_host, self.proxy_port))
            ...
            self.log('wes ok mbah[G1]'.format(self.proxy_host, self.proxy_port), color='[P1]')
        except OSError:
            self.log('BERMASALAH', color='[R1]')
        except TimeoutError:
            self.log('{} TIDAK ADA RESPON'.format(self.proxy_host), color='[R1]')

def main():
    print(colors('\n'.join([
        '[R2]AHYAT INJECT',
        '[CC]       ++SCRIPT TELKOMSEL++ ', '[CC]'
    ])))
    inject('127.0.0.1', '8899').start()
